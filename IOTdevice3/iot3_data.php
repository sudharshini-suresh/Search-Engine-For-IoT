...
// 		$db_name = 'cfavouri_mirrordb';
	...
	if($input_params["gas"] != '2000'){
	   // echo $input_params["temperature"];
		$sql = "update temp_iot3 set  gas= '{$input_params["gas"]}'";
		//echo $sql;
...
//		return $res;
	...
	if($input_params["location"] != '2000'){
		$sql = "update temp_iot3 set location = '{$input_params["location"]}'";
		$res = mysqli_query($con,$sql);
//		return $res;
	}
...
$time=date("h:i:s A");
	...
	$sql = "INSERT INTO te_graph (gas,location,date1,time) VALUES ( '{$input_params["gas"]}','{$input_params["location"]}', '$date','$time') ";
...
// 		$db_name = 'cfavouri_mirrordb';
	...
	if( isset($input_params['gas']) && $input_params['gas'] != '' && $input_params['gas'] != NULL)
	{
		$status = false;
